...
TrajLAMMPS::TrajLAMMPS(const char* filename, const char* mode) : Traj()
{
    ...
    attrs['s'] = (cstate!=-1);
    attrs['l'] = (cl!=-1);
    attrs['w'] = (cvl!=-1); // change to w
    attrs['g'] = (cfl!=-1); // change to g
    attrs['n'] = (cml!=-1); // change to n
...
int TrajLAMMPS::read_body()
{
    if(natoms > _natoms)
    {
        ...
        memset(&(s[0]), 0, sizeof(int)*natoms);
        memset(&(l[0]), 0, sizeof(float)*natoms);
        memset(&(vl[0]), 0, sizeof(float)*natoms);
        memset(&(fl[0]), 0, sizeof(float)*natoms);
        memset(&(ml[0]), 0, sizeof(float)*natoms);
    }
